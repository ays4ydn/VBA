document.addEventListener("DOMContentLoaded", () => {
    ...
    window.mountBroom = function() {
        ...
        if (toy) {
            toy.style.transition = 'transform 0.4s ease-in, opacity 0.4s';
            toy.style.transform = 'scale(0) translateY(-150px) rotate(180deg)';
            toy.style.opacity = '0';
            setTimeout(() => { toy.style.display = 'none'; }, 400); // Remove from DOM calculations
        }
    };
    ...
    function animateEvasiveButtons() {
        ...
        if (struggleFrames > 5 && !buttonsFrozen) { // Anında müdahale için sadece 5 frame hata payı!
            ...
            helperCat.bubble.classList.add('show'); // CSS class ile pointer-events aç
            
            // Butonları çok daha kısa süre (1.6s) dondurup art arda daha sık çıkmasını sağla
            setTimeout(() => {
                buttonsFrozen = false;
                if (helperCat) {
                    helperCat.bubble.classList.remove('show');
                    helperCat.isHelping = false; 
                }
            }, 2500); // Kullanıcı daha rahat tıklayabilsin diye süreyi biraz uzattık (2.5s)
        }
    }
        ...
        if (currentStage === 1) {
            ...
            if (harryToy && harryBubble) {
                ...
                if (dist < 250 && mouseX !== 0) {
                    ...
                    const flip = mouseX < hX ? -1 : 1;
                    harryToy.style.transform = `translateX(-50%) scaleX(${flip})`;
                    // KONUŞMA BALONUNUN TERS DÖNMESİNİ ENGELLER (Counter-transform)
                    harryBubble.style.transform = `translateX(-50%) scaleX(${flip})`; 
                    
                    // Voltasını dondur ve otur!
                    harryWait = 10;
                    ...
                    if (harryWait <= 0) {
                        ...
                        harryX += harryDirection * 0.15; 
                        
                        // Uca geldiğinde yorulur ve oturur (Wait)
                        if (harryX > 85) { harryDirection = -1; harryWait = 100; harryToy.classList.remove('walking'); harryToy.classList.add('sitting'); }
                        if (harryX < 15) { harryDirection = 1; harryWait = 100; harryToy.classList.remove('walking'); harryToy.classList.add('sitting'); }
                        
                        harryToy.style.left = `${harryX}%`;
                        const flip = harryDirection === -1 ? -1 : 1;
                        harryToy.style.transform = `translateX(-50%) scaleX(${flip})`;
                        harryBubble.style.transform = `translateX(-50%) scaleX(${flip})`;
                    }
                }
